...
for fold in range(10):
    ...
    print(f"Fold {fold+1} Train: {len(set_train)} Valid: {len(set_valid)}")

    train_dataset = torchvision.datasets.DatasetFolder(root="../datasets/downstream/sleep_edf_full/TrainFold", loader=lambda x: torch.load(x),  extensions=['.pt'])
    valid_dataset = torchvision.datasets.DatasetFolder(root="../datasets/downstream/sleep_edf_full/TestFold", loader=lambda x: torch.load(x), extensions=[f'.pt'])
